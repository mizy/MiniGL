...
class MiniGLTextureAtlasData extends TextureAtlasData {
    ...
    setRenderTexture(texture, miniGL) {
        ...
        let textureUtil = new Texture(miniGL);
        ...
        if (this._renderTexture !== null) {
            for (let k in this.textures) {
                ...
                textureData.renderTexture = textureUtil.shaderProgram{image: texture, rect: textureData.region, reverseY: false, name: textureData.name});
            }
        } else {
            for (let k in this.textures) {
                const textureData = this.textures[k];
